...
Ensure the Home Assistant MQTT integration and a broker are available, then configure MQTT_HOST with the broker's real hostname or LAN IP, plus credentials and optional TLS in .env. The discovery prefix defaults to homeassistant. Leave MQTT_HOST empty to collect and store prices without publishing to Home Assistant.
